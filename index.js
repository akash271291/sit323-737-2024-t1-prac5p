...
app.get('/division', function (req,res){ 
    try{
      let num1 = parseFloat(req.query.num1); //storing the value of num1 from the request body to variable num1
        let num2 = parseFloat(req.query.num2); //storing the value of num2 from the request body to variable num2
        if(isNaN(num1) || num1 == null || num1 == '') //checking if num1 is empty or null, if yes it will throw error that num1 is incorrect
            throw new Error("Number 1 is incorrect");    
        if(isNaN(num2) || num2 == null || num2 == '') //checking if num2 is empty or null, if yes it will throw error that num2 is incorrect
            throw new Error("Number 2 is incorrect");    
            logger.info('Value of num1 is: '+num1+' and value of num2 is: '+num2);
            const result = division(num1, num2);
            res.status(200).send(`<p>Result of division of Num1 and Num2 is: `+result);
...
app.get('/multiplication', function (req,res){
    try{
      let num1 = parseFloat(req.query.num1); //storing the value of num1 from the request body to variable num1
        ...
        res.status(200).send(`<p>Result of multiplication of Num1 and Num2 is: `+result);
...
app.get('/addition', function (req,res){
    try{
        ...
        if(isNaN(num2) || num2 == null || num2 == '') //checking if num2 is empty or null, if yes it will throw error that num2 is incorrect
            throw new Error("Number 2 is incorrect");    
         //storing the value of num2 from the request body to variable num2
       // logOperation('addition', num1, num2); //Calling logOperation method to log the arithmetic operation
        ...
        //res.status(200).json({ statuscode: 200, data: result });
        res.status(200).send(`<p>Result of Addition of Num1 and Num2 is: `+result);
    }catch(err){
        logger.error(err.toString());
       // res.status(500).json({
         // statuscode: 500, msg: err.toString()
        //})
        res.status(500).send(`<h3>${ err.toString() }</h3> <a href='/'>Home</a>`);//If there is an error in try block then it will set the status to 500 and send the error string
    }
});
    ...
    try{
      let num1 = parseFloat(req.query.num1); //storing the value of num1 from the request body to variable num1
       ...
       // logOperation('subtraction', num1, num2); //Calling logOperation method to log the arithmetic operation
        ...
        //res.status(200).json({ statuscode: 200, data: result });
        res.status(200).send(`<p>Result of subtraction of Num1 and Num2 is: `+result);
      }catch(err){
        logger.error(err.toString());
        res.status(500).send(`<h3>${ err.toString() }</h3> <a href='/'>Home</a>`);//If there is an error in try block then it will set the status to 500 and send the error string
    }
